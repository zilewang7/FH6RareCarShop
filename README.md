...
Windows x64 下的 Forza Horizon 6 多活动奇珍商店库存选择与重购交互恢复工具。
当前版本为 **2.0.0**，支持 Xbox 应用版与 Steam 版。
...
工具默认选择首次开放时间最晚的活动，也可在顶部切换历史活动。时间范围用于标识活动
首次出现的赛季，不表示活动在未来不会复刻，也不能替代游戏内倒计时。

选择活动后，工具要求游戏内管理器的槽数、完整车池及车型映射与该活动全部一致。游戏
当前开放的是英国奇珍却选择意大利奇珍时，工具会拒绝写入；历史活动的目录不能绕过
游戏当前活动门禁。
...
源码也可作为后续同类奇珍活动工具的参考基座。可复用部分包括活动化目录、完整结构
不变量定位、版本画像与回退、受校验的游戏函数调用、远程线程状态管理和事务式回滚。

## 与随机刷新的关系

这个工具不会修改存档、车库、货币或网络请求，也不会直接向账号发放车辆。普通库存
切换调用游戏自身的展位 setter；恢复重购还会调用游戏自身的完整刷新流程，候选车辆
始终来自所选活动原有的对应展位车池。

从游戏与工具日志能观察到的结果看，游戏只是完成了一次正常活动刷新，并正好“随机”
到了所选车辆。最终效果与完全退出游戏、重新进入并反复等待随机命中没有区别，工具只
是省去了大退和等待时间。

截至已验证版本，未观察到 FH6 为此流程部署独立反作弊拦截。这不代表官方授权、永久
兼容或零风险；游戏更新后，工具会在活动结构或代码特征校验失败时停止写入。
